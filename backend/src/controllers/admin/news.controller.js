...
const { News, User, BusCompany, sequelize } = require('../../../models');
...
const enrichCompanyTags = async (tags, companyId) => {
  if (!companyId || !(await newsHasCompanyId()) || !BusCompany) {
  ...
  try {
    ...
    const companyTags = [
      company.name ? `Nhà xe ${company.name}` : null,
      company.code ? `company:${company.code}` : `company:${company.id}`
    ].filter(Boolean);

    const combined = [...tags, ...companyTags];
    return Array.from(new Set(combined));
  } catch (error) {
    console.warn('Unable to enrich company tags:', error.message);
    return Array.from(new Set(tags));
  }
};
...
const createNews = async (req, res) => {
  try {
    ...
    const data = { title: title.trim(), slug, content, summary: summary?.trim(), category, status, featuredImage, isHighlighted, authorId: req.user.id };

    let companyId = null;
    if (await newsHasCompanyId()) {
    ...
    data.tags = await enrichCompanyTags(baseTags, companyId);
...
const updateNews = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, content, summary, category, status, featuredImage, tags, isHighlighted, publishNow, companyId: requestedCompanyId } = req.body;
    if (!id) return res.status(400).json({ success: false, message: 'Thiếu mã tin tức' });

    const item = await News.findByPk(id);
    if (!item) return res.status(404).json({ success: false, message: 'Không tìm thấy tin tức' });

    const hasCompanyColumn = await newsHasCompanyId();
    if (hasCompanyColumn && req.user && req.user.role === ROLES.COMPANY && item.companyId && item.companyId !== req.user.companyId) {
      return res.status(403).json({ success: false, message: 'Không có quyền chỉnh sửa tin tức của nhà xe khác' });
    }
    ...
    if (title) {
      ...
      if (title.trim() !== item.title) {
        ...
        let candidate = base;
        let counter = 1;
        while (await News.findOne({ where: { slug: candidate, id: { [Op.ne]: id } } })) {
    ...
    if (hasCompanyColumn) {
      ...
      } else if (requestedCompanyId !== undefined) {
        const parsedCompanyId = Number(requestedCompanyId);
        targetCompanyId = Number.isNaN(parsedCompanyId) ? null : parsedCompanyId;
        update.companyId = targetCompanyId;
      }
    }

    const existingTags = Array.isArray(item.tags) ? item.tags : [];
    const incomingTags = tags !== undefined ? normalizeTags(tags) : existingTags;
    update.tags = await enrichCompanyTags(incomingTags, targetCompanyId);

    if (publishNow || status === 'PUBLISHED') {
      update.status = 'PUBLISHED';
      if (!item.publishedAt) update.publishedAt = new Date();
    }

    await item.update(update);
    const include = [{ model: User, as: 'author', attributes: ['id', 'name', 'email'] }];
    if (hasCompanyColumn && sequelize.models.BusCompany) {
      include.push({ model: sequelize.models.BusCompany, as: 'company', attributes: ['id', 'name', 'code'] });
    }
